...
def _watch_pipeline_job(job_id: str, process: subprocess.Popen, input_filename: str, row_count: int,
    ...
    try:
        ...
        with _pipeline_jobs_lock:
            ...
            if job.get("status") == "quota_exhausted":
                job.update({"return_code": return_code, "finished_at": datetime.now(timezone.utc).isoformat()})
                logger.warning("Detached pipeline job %s withdrawn after Gemini quota exhaustion.", job_id)
                return
            job.update({
                "status": status,
...
def pipeline_job_status(job_id: str):
    ...
    if job.get("status") == "running":
        stderr_path = PROJECT_ROOT / str(job.get("stderr_log", ""))
        try:
            log_text = stderr_path.read_text(encoding="utf-8", errors="replace")
            # A sustained run of quota responses with no successful Gemini call
            # is not meaningful progress; withdraw it from the UI explicitly.
            if log_text.count("HTTP/1.1 429") >= 8 and "HTTP/1.1 200 OK" not in log_text:
                job = dict(job)
                job.update({
                    "status": "quota_exhausted",
                    "message": "Gemini quota exhausted; pipeline withdrawn. Use a key with available quota and start a fresh run.",
                })
                pid = job.get("pid")
