...
def get_last_llm_provider() -> str:
    return getattr(_provider_local, "name", "gemini")


def _set_llm_provider(name: str) -> None:
    _provider_local.name = name
...
def _call_gemini_rest(
    ...
    if gemini_failed:
        logger.info("Gemini exhausted %d/%d attempts — falling through to Groq.", attempts_used, 3)
        groq_key = get_groq_api_key()
        if groq_key:
            groq_text = _call_groq_rest(prompt, groq_key, timeout=timeout)
            if groq_text:
                return groq_text
    if last_exc:
...
def _call_groq_rest(prompt: str, api_key: str, timeout: float = 30.0, retries: int = 2) -> Optional[str]:
    """Use Groq only after Gemini quota/rate exhaustion."""
    for attempt in range(retries):
        try:
            resp = httpx.post(
                GROQ_API_URL,
            ...
            if resp.status_code == 200:
                ...
                if content:
                    _set_llm_provider("groq-fallback")
                    return content
            if resp.status_code == 429:
                logger.warning("Groq HTTP 429 on fallback attempt %d/%d", attempt + 1, retries)
                time.sleep((attempt + 1) * 2.0)
                continue
            logger.warning("Groq fallback HTTP %d: %s", resp.status_code, resp.text[:300])
            return None
        except (httpx.TimeoutException, httpx.TransportError, httpx.NetworkError) as exc:
            logger.warning("Groq fallback network error (attempt %d/%d): %s", attempt + 1, retries, exc)
            time.sleep((attempt + 1) * 2.0)
    return None
    ...
    def _process_batch(batch: List[Tuple[int, str]]) -> List[Tuple[int, str, str, str]]:
        ...
        for i, item in enumerate(parsed):
            ...
            if full_path:
                ...
                out.append((batch[i][0], full_path, dept, fine, provider))
        return out
    ...
    with ThreadPoolExecutor(max_workers=max_workers) as executor:
        ...
        for future in as_completed(futures):
            try:
                ...
                for row_idx, full_path, dept, fine, provider in batch_results:
                    results[row_idx] = (full_path, dept, fine, provider)
...
def extract_dynamic_attributes_batch(
    ...
    def _process_batch(batch: List[Tuple[int, str, str, str]]) -> Dict[int, Dict[str, Dict[str, Optional[str]]]]:
        ...
        for i, item in enumerate(parsed):
            ...
            if attrs:
                attrs["__llm_provider__"] = provider
                out[row_idx] = attrs
        return out
...
def generate_descriptions_batch(items: List[Dict[str, Any]], batch_size: int = 20,
    ...
    def _run(batch: List[Dict[str, Any]]) -> Dict[int, Dict[str, str]]:
        ...
        for idx, obj in enumerate(parsed):
            ...
            if vals:
                vals["__llm_provider__"] = provider
