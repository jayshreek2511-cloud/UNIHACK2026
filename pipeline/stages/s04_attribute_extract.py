    ...
    for label, data in all_raw.items():
        ...
        if llm_attrs and label in llm_attrs and llm_attrs[label].get("value"):
            if re.search(re.escape(str(value)), desc, re.IGNORECASE):
                ...
                conf = "llm-inferred (groq-fallback)" if llm_provider == "groq-fallback" else "llm-inferred"
...
def _extract_generic_attributes(
    ...
    provider = "gemini"
    if llm_attrs:
        provider = str(llm_attrs.get("__llm_provider__", "gemini"))
    return _merge_and_tag(desc, generic_attrs, llm_attrs, llm_provider=provider)
