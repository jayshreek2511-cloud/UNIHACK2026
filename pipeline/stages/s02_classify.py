...
def classify(
    ...
    if llm_results:
        for rec in records:
            ...
            if res:
                full_path, dept, fine = res[:3]
                provider = res[3] if len(res) > 3 else "gemini"
                rec.coarse_category = full_path
                rec.classification_method = "llm-classified (groq-fallback)" if provider == "groq-fallback" else METHOD_LLM
                llm_classified += 1
